    ...
    pub fn new() -> Self {
        ...
        if m_path.exists() {
            if let Ok(data) = std::fs::read_to_string(&m_path) {
                if let Ok(missions) =
                    serde_json::from_str::<HashMap<String, super::MissionMetadata>>(&data)
                {
                    map = missions;
                    println!(
                        "[V2.4] Mission Registry Restored: {} active missions loaded.",
                        map.len()
                    );
                }
            }
...
impl AiManager {
    ...
    fn create_provider(config: &crate::config::AiConfig, name: &str) -> Box<dyn AiProvider> {
        match name {
            ...
            "openai" | "gpt-4o" => {
                ...
                Box::new(super::cloud::OpenAiProvider::new(
                    &key,
                    name,
            ...
            "claude" => {
                ...
                Box::new(super::cloud::ClaudeProvider::new(
                    &key,
                    name,
                    &config.cloud.endpoint,
                    config.cloud.timeout_ms,
            ...
            "gpt-oss" => {
                ...
                Box::new(super::cloud::GptOssProvider::new(
                    &key,
            ...
            "huggingface" | "hf" => {
                let key = config
                    ...
                    .api_key
                    .clone()
                    .unwrap_or_else(|| std::env::var("EDGECLAW_HF_KEY").unwrap_or_default());
                Box::new(super::huggingface::HuggingFaceProvider::new(
    ...
    pub fn process(&self, request: &AiRequest) -> Result<AiResponse, AgentError> {
        ...
        {
            println!(
                "[V2.4] No mission in primary intent. Scanning response text for embedded JSON..."
            );
            let mut cursor = 0;
            while let Some(start) = response.message[cursor..].find('{') {
                ...
                if let Some(end) = response.message[start_abs..].rfind('}') {
                    ...
                    if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(candidate) {
                        ...
                        if let Some(m_val) = mission_val {
                            if let Ok(mut m) =
                                serde_json::from_value::<super::MissionMetadata>(m_val)
                            {
                ...
                cursor += start + 1;
                if cursor >= response.message.len() {
                    break;
                }
            }
        }
    ...
    fn process_consensus(&self, request: &AiRequest) -> Result<AiResponse, AgentError> {
        ...
        println!(
            "[Fleet Ensemble] Domain detected: '{}', assigning {} expert roles",
            domain,
            expert_roles.len()
        );
        ...
        println!(
            ...
            consensus_level, best_response.role, best_response.model
        );
        ...
        let sub_responses: Vec<AiResponse> = expert_responses
            .iter()
            .map(|er| {
    ...
    pub fn active_mission(&self) -> Option<super::MissionMetadata> {
        ...
        let active = lock
            .values()
            .find(|m| m.status == super::MissionStatus::Active)
            .cloned();
        if active.is_some() {
            return active;
        }
        ...
        let planning = lock
            .values()
            .find(|m| m.status == super::MissionStatus::Planning)
            .cloned();
        if planning.is_some() {
            return planning;
        }
        ...
        let discovery = lock
            .values()
            .find(|m| m.status == super::MissionStatus::Discovery)
            .cloned();
