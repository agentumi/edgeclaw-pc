...
impl ChainProvider for SolanaProvider {
    ...
    fn revoke_policy(&self, _policy_id: &str) -> Result<ChainTxResult, AgentError> {
        Ok(ChainTxResult {
            ...
            chain: ChainType::Solana, block_number: Some(250_000_000), gas_used: 5000, success: true,
    ...
    fn anchor_audit(&self, batch_start: u64, batch_end: u64, batch_hash: &str) -> Result<ChainAuditAnchor, AgentError> {
        Ok(ChainAuditAnchor {
            ...
            tx: ChainTxResult {
                tx_hash: hex::encode(&uuid::Uuid::new_v4().as_bytes()[..]),
                chain: ChainType::Solana, block_number: Some(250_000_001), gas_used: 5000, success: true,
                timestamp: chrono::Utc::now().timestamp() as u64,
            }
        })
    }
    ...
    fn get_balance(&self, _address: &str) -> Result<ChainBalance, AgentError> {
        Ok(ChainBalance { chain: ChainType::Solana, symbol: "SOL".to_string(), amount: 0, decimals: 9 })
    }
    fn status(&self) -> ChainProviderStatus {
        ChainProviderStatus {
            chain: ChainType::Solana, connected: self.connected,
